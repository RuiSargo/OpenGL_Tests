...
int main() {
    ...
    // Carregar shaders
    std::string vertexShaderSource = loadShaderSource("shaders/vertex_shader_art_v2.glsl");
    std::string fragmentShaderSource = loadShaderSource("shaders/fragment_shader_art_v2.glsl");
    ...
    // Configuração do framebuffer e texture buffer
    GLuint framebuffer;
    ...
    if(glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
        std::cerr << "Framebuffer não está completo!" << std::endl;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Renderizar a cena no framebuffer
        ...
        // Aplicar o filtro ao conteúdo renderizado
        glClear(GL_COLOR_BUFFER_BIT);
        glUseProgram(shaderProgram);
